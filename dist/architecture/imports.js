...
import { SyntaxKind, isCallExpression, isExportDeclaration, isExternalModuleReference, isIdentifier, isImportDeclaration, isImportTypeNode, isLiteralTypeNode, isStringLiteral, isNoSubstitutionTemplateLiteral, isVariableDeclaration } from "typescript/unstable/ast";
import { infrastructure, inside, matches, relativePath, sourceFile } from "./paths.js";
export const checkImports = (root, files, projects) => {
    ...
    for (const file of files) {
        for (const project of projects) {
            ...
            const checkTarget = (node, target) => {
                const actual = realpathSync(target);
                const classified = byPath.get(actual);
                if (!inside(root, actual)) {
                    report(node, `Import target escapes architecture root: ${target}`);
                    return;
                }
                if (classified === undefined) {
                    report(node, `Import target is not governed source: ${relativePath(root, actual)}`);
                    return;
                }
                if (!file.policy.imports.internal.includes(classified.type))
                    report(node, `File type ${file.type} may not import ${classified.type}: ${classified.relative}`);
            };
            const check = (node) => {
                if (!isStringLiteral(node) && !isNoSubstitutionTemplateLiteral(node)) {
                    report(node, "Nonliteral module loading cannot be verified. Use a literal import or a documented line exception.");
                    return;
                }
                const specifier = node.text;
                if (isBuiltin(specifier)) {
                    if (!matches(specifier.replace(/^node:/, ""), file.policy.imports.builtins))
                        report(node, `Built-in import is not permitted: ${specifier}`);
                    return;
                }
                ...
                if (local && !sourceFile(asset) && existsSync(asset)) {
                    ...
                    if (!inside(root, actual) || !matches(relativePath(root, actual), file.policy.imports.assets))
                        report(node, `Asset import is not permitted: ${specifier}`);
                    return;
                }
                ...
                if (!matches(specifier, file.policy.imports.external))
                    report(node, `External import is not permitted: ${specifier}`);
            };
            const visit = (node) => {
                if (isImportDeclaration(node) || isExportDeclaration(node)) {
                    ...
                    if (isIdentifier(node.expression) && node.expression.text === "require") {
                        ...
                        const createdRequire = localDefinitions.some((declaration) => {
                            ...
                            if (binding === undefined || !isVariableDeclaration(binding) || binding.initializer === undefined || !isCallExpression(binding.initializer))
                                return false;
                            const factory = binding.initializer.expression;
                            if (!isIdentifier(factory))
                                return false;
                            ...
                            return factorySymbol?.declarations.some((handle) => {
                                ...
                                while (definition !== undefined && definition.kind !== SyntaxKind.SourceFile) {
                                    if (isImportDeclaration(definition))
                                        return isStringLiteral(definition.moduleSpecifier) && ["node:module", "module"].includes(definition.moduleSpecifier.text);
                                    definition = definition.parent;
                                }
                                return false;
                            }) ?? false;
                        });
                        if (localDefinitions.length === 0 || createdRequire)
                            check(node.arguments[0] ?? node);
                    }
                }
...
const checkReferences = (source, file, report, checkTarget) => {
    for (const reference of source.referencedFiles) {
        ...
        if (existsSync(target))
            ...
            report(source, `Unresolved reference path: ${reference.fileName}`);
    }
    for (const reference of source.typeReferenceDirectives) {
        if (!matches(reference.fileName, file.policy.imports.external))
            report(source, `Type reference is not permitted: ${reference.fileName}`);
    }
};
