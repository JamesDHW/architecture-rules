import { getLeadingCommentRanges, getTrailingCommentRanges, SyntaxKind, isJsxExpression } from "typescript/unstable/ast";
/** Policy validation happens before lint, so a broad disable cannot disable this check. */
export const checkSuppressions = (files, projects) => {
    const issues = [];
    for (const file of files) {
        ...
        const visit = (node) => {
            ...
            for (const comment of [...leading, ...trailing, ...jsxComments])
                comments.set(comment.pos, comment.end);
            node.forEachChild((child) => { visit(child); });
        };
        visit(source);
        for (const [start, end] of comments) {
            const text = source.text.slice(start, end).replace(/^\/[/\*]\s*/, "").replace(/\*\/$/, "").trim();
            if (!/^(?:eslint|oxlint)(?:-|\s)/.test(text))
                continue;
