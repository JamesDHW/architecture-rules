...
const implementation = {
    meta: {
        ...
        schema: [],
        messages: {
            requireBraces: "Use braces except for single-line terminal if guards. See rule control-flow-braces.",
        },
    },
    create(context) {
        return {
            IfStatement(node) {
                if (node.consequent.type === "BlockStatement")
                    return;
                const isTerminal = node.consequent.type === "ReturnStatement" ||
                    node.consequent.type === "ThrowStatement" ||
                    node.consequent.type === "BreakStatement" ||
                    node.consequent.type === "ContinueStatement";
                const isSingleLine = node.loc !== undefined && node.loc !== null &&
                    node.loc.start.line === node.loc.end.line;
                if (node.alternate === null && isTerminal && isSingleLine)
                    return;
...
//# sourceMappingURL=controlFlowBraces.rule.js.map