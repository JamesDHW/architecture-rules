...
const naming = (value) => {
    ...
    if (input.case !== "camel" && input.case !== "pascal" && input.case !== "pascalOrCamel")
        throw new Error("Invalid naming.case");
    const allowedNames = input.allowedNames === undefined ? undefined : strings(input.allowedNames, "allowedNames");
    if (allowedNames?.some((name) => /[/\\*?{}[\]]/.test(name)))
        throw new Error("allowedNames must be exact basenames");
    return {
        case: input.case,
...
const imports = (value, names) => {
    ...
    for (const name of internal)
        if (!names.includes(name))
            throw new Error(`Unknown internal file type: ${name}`);
    return {
        internal,
...
export const isArchitecture = (value) => value !== null && typeof value === "object" && definitions.has(value);
...
export const normalizeArchitecture = (value) => {
    ...
    if (tsconfigs.length === 0)
        throw new Error("At least one tsconfig is required");
    const references = projects.references ?? "follow";
    if (references !== "follow" && references !== "explicit")
        throw new Error("Invalid projects.references");
    const types = fields(config.fileTypes, Object.keys((config.fileTypes ?? {})), "fileTypes");
    const names = Object.keys(types);
    if (names.length === 0 || names.some((name) => !/^[A-Za-z][\w-]*$/.test(name) || ["__proto__", "constructor", "prototype"].includes(name)))
        throw new Error("fileTypes requires valid, non-reserved names");
    const fileConfig = fields(config.files ?? {}, ["otherFiles", "toolingFiles", "generated"], "files");
    const generated = fileConfig.generated ?? [];
    if (!Array.isArray(generated))
        throw new Error("generated must be an array");
    const files = {
    ...
    for (const [name, value] of Object.entries(types)) {
        ...
        if (include.length === 0)
            throw new Error(`${name}.files must not be empty`);
        const policy = {
    ...
    const architecture = freeze({ projects: { tsconfigs, references }, files, defaults, fileTypes });
    definitions.add(architecture);
    return architecture;
};
export const defineArchitecture = (config) => normalizeArchitecture(config);
//# sourceMappingURL=defineArchitecture.js.map