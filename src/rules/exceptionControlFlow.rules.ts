...
const rawImplementation: Rule.RuleModule = {
  meta: {
  ...
  create(context) {
    return {
      "CatchClause, ThrowStatement"(node: Rule.Node) { context.report({ node, messageId: "raw" }); },
    };
  },
};
...
const finallyImplementation: Rule.RuleModule = {
  meta: {
