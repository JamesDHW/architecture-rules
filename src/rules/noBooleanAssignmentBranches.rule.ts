...
const isInBranch = (node: Rule.Node): boolean => {
  let child = node;
  ...
  while (parent !== undefined && parent !== null) {
    if (["FunctionExpression", "ArrowFunctionExpression", "FunctionDeclaration"].includes(parent.type)) {
      if (parent.parent?.type !== "CallExpression" || parent.parent.callee !== parent) return false;
    }
    if (parent.type === "SwitchCase" && parent.test !== child) return true;
    if (parent.type === "IfStatement" && parent.test !== child) return true;
    if (parent.type === "ConditionalExpression" && parent.test !== child) return true;
    child = parent;
  ...
  create(context) {
    return {
      AssignmentExpression(node) {
        ...
        if (isInBranch(node)) context.report({ node, messageId: "booleanAssignment" });
      },
    };
  },
};
