...
export const namingError = (path: string, policy: NamingPolicy): string | undefined => {
  const name = basename(path);
  if (policy.allowedNames?.includes(name)) return undefined;
  let stem = name.replace(/(?:\.d)?\.[cm]?[jt]sx?$/i, "");
  const suffix = [...(policy.suffixes ?? [])].sort((a, b) => b.length - a.length).find((part) => stem.endsWith(part));
  if (suffix !== undefined) stem = stem.slice(0, -suffix.length);
  ...
  if (!valid) return `Filename '${name}' must use ${policy.case} case${suffix === undefined ? "" : ` before ${suffix}`}.`;
  if (policy.prefix !== undefined && !stem.startsWith(policy.prefix)) return `Filename '${name}' must start with '${policy.prefix}'.`;
  return undefined;
};
