...
import { inventory, classify, type Issue } from "./inventory.js";
...
const execute = (args: readonly string[], cwd: string): Promise<number> => new Promise((finish) => {
  const child = spawn(process.execPath, [oxlint, ...args], { cwd, stdio: "inherit" });
  child.on("error", (error) => { process.stderr.write(`${error.message}\n`); finish(1); });
  child.on("close", (code) => finish(code ?? 1));
});
export const printIssues = (issues: readonly Issue[]): void => {
  for (const issue of issues) process.stderr.write(`${issue.file}${issue.line === undefined ? "" : `:${issue.line}`}: ${issue.ruleId}: ${issue.message}\n`);
};

export const checkArchitecture = async (root: string, architecture: Architecture, options: { readonly fix?: boolean; readonly explain?: string } = {}): Promise<number> => {
  ...
  try {
    ...
    if (structural.length > 0) { printIssues(structural); return 1; }
    ...
    try {
      ...
      for (const project of checked.projects) {
        ...
        for (const diagnostic of diagnostics) {
          if (diagnostic.category !== 1) continue;
    ...
    } finally { if (checked !== opened) checked.close(); }
